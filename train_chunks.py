...
def run_query_pg(sql, bao_select, bao_reward):
    plans = []
    try:
        db = Database('imdb')
        db.set_timeout(240000)
        plan = db.execute(sql, bao_select, bao_reward)
        plans.append({'Query': fp, 'Plan': plan})
        db.close()
    except:
        plans.append({'Query': fp, 'Plan': 'timeout'})
...
pg_chunks = queries[:256]
training_set, testing_set = train_and_test_set_split(queries[256:], 0.8)
...
# PG预热
pg_plans = []
pg_result_path = '/data1/linliang/Bao/BaoForPostgreSQL/data_train_2/pg.json'
for fp, q in pg_chunks:
    pg_plans.append(run_query_pg(q, bao_select=USE_PG, bao_reward=USE_BAO))
   
with open(pg_result_path, 'w') as f:
    json.dump(pg_plans, f)
print("PG Initing Results have already been written to file:", pg_result_path)



# 训练
train_chunks = list(chunks(training_set,256))
train_plans = []
for idx, chunk in enumerate(train_chunks):   #用bao进行训练
    i = 0
    if USE_BAO:
    ...
    for q_idx, (fp, q) in enumerate(chunk):
        train_plans.append(run_query(q, bao_select=USE_BAO, bao_reward=USE_BAO))
        print(train_plans[i])
        i += 1

train_res_path = '/data1/linliang/Bao/BaoForPostgreSQL/data_train_2/bao_train.json'
with open(pg_result_path, 'w') as f:
    json.dump(train_plans, f)
print("Training Results have already been written to file:", train_res_path)
...
test_plans = []
if USE_BAO:
    os.system("cd bao_server && /data1/linliang/anaconda3/envs/py310/bin/python3 baoctl.py --load /data1/linliang/Bao/BaoForPostgreSQL/bao_server/bao_default_model")
    os.system("sync")
...
test_path = '/data1/linliang/Bao/BaoForPostgreSQL/data_train_2/bao_test.json'
with open(test_path, 'w') as f:
    json.dump(test_plans, f)

print("Testing Results have already been written to file:", test_path)
