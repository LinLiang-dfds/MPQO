...
for c_idx, chunk in enumerate(training_set):
    ...
    for q_idx, (fp, q) in enumerate(chunk):
        ...
        print(c_idx, q_idx, fp, q_time, q_memory, flush=True)
...
for c_idx, chunk in enumerate(testing_set):
    for q_idx, (fp,q) in enumerate(chunk):
        q_time, q_memory = run_query(q,bao_reward=USE_BAO, bao_select=USE_BAO)
        test_time.append((fp, q_time))
        test_memory.append((fp, q_memory))
        print("Testing Query:",c_idx, q_idx, fp, q_time, q_memory, flush = True)
