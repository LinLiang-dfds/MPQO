...
class BaoData:
    ...
    def __getitem__(self, idx):
        return (self.__data[idx]["tree"],   #tree是键
                self.__data[idx]["target"])  #target是键
...
class BaoRegression:
    def __init__(self, verbose=False, have_cache_data=False):
        ...
        scale_transformer = preprocessing.MinMaxScaler() #对数据进行最小-最大缩放

        self.__pipeline = Pipeline([("log", log_transformer),       #对目标进行对数变换
                                    ("scale", scale_transformer)])  #对目标变量进行最小-最大缩放
    ...
    def load(self, path):
        with open(_n_path(path), "rb") as f:           #加载保存的样本的数量
            self.__n = joblib.load(f)
        with open(_channels_path(path), "rb") as f:    #加载输入通道数
    ...
    def save(self, path):
        ...
        torch.save(self.__net.state_dict(), _nn_path(path))     #将pytorch模型的状态保存到磁盘，以便稍后加载和使用
